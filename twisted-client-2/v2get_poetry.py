...
class PoetryProtocol(Protocol):
    ...
    def dataReceived(self, data):
        ...
        #timeout is not none and data is not empty
        if self.timeout and data and self.timeout.active():
            self.timeout.cancel()
        ...
        if not self.finish and time.time() - self.start >= 2:
            self.timeout = reactor.callLater(2, self.transport.write)
